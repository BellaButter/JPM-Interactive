...
vec4 createMotionBlur(vec2 uv, vec4 baseColor, vec2 velocity) {
  ...
  const int MAX_MOTION = 20;
  float n = max(1.0, float(uMotionSample));
  for (int i = 1; i < MAX_MOTION; i++) {
    float fi = float(i);
    float t = fi / max(1.0, n - 1.0);
    vec2 offset = t * scaledV / uResolution;
    float w = step(fi, n - 0.5);
    baseColor += w * texture2D(uBuffer, uv + offset);
  }
  return baseColor / n;
}
...
void main() {
  ...
  vec4 brushColor = uIsTexture ? texture2D(uTexture, uv) : vec4(uColor, uBrushAlpha);
  float onLine = isOnLine(suv, uPrevMouse, uMouse, radius, uPressureStart, uPressureEnd);
  float lineMask = length(velocity) > 0. ? onLine : uIsCursor ? onLine : 0.;
  vec4 finalColor = mix(bufferColor, brushColor, lineMask);
