...
export type UniformObject = { [key: string]: { value: UniformValue } };
...
export const setUniform =
  ...
  (key: keyof T, value: UniformValue) => {
    ...
    if (uniforms && uniforms[key]) {
      (uniforms[key] as { value: UniformValue }).value = value;
    }
  };
...
export const setCustomUniform =
  ...
  (customParams: CustomParams | undefined) => {
    if (customParams === undefined) return;
    Object.keys(customParams).forEach((key) => {
      const uniforms = material.uniforms;
      if (uniforms && uniforms[key]) {
        (uniforms[key] as { value: UniformValue }).value = customParams[key];
      }
    });
  };
